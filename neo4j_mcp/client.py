...
class Neo4jClient:
    ...
    async def create_node(self, node: NodeCreate) -> Node:
        ...
        async with session:
            result = await session.run(
                query,  # ty: ignore[invalid-argument-type] node.properties)
    ...
    async def get_node(self, node_id: str) -> Node | None:
        ...
        async with session:
            result = await session.run(
                query,  # ty: ignore[invalid-argument-type] {"id": int(node_id)}  # ty: ignore[invalid-argument-type]
            )
            record = await result.single()
            if record:
                return Node(
    ...
    async def delete_relationship(self, rel_id: str) -> bool:
        ...
        async with session:
            await session.run(
                query,  # ty: ignore[invalid-argument-type] {"id": int(rel_id)})
            return True
